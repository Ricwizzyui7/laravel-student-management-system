                        ...
                        @forelse($rows as $row)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-900/50 transition-colors">
                                <td class="py-3.5 px-6">
                                    <a href="{{ route('attendance.student', $row['student']->id) }}" class="font-medium text-gray-900 dark:text-gray-100 hover:text-blue-600 hover:underline transition">
                                        {{ $row['student']->fullname }}
                                    </a>
                                    <div class="text-xs text-gray-400 dark:text-gray-500">{{ $row['student']->course }}</div>
                                </td>
                                <td class="py-3.5 px-6 text-center text-gray-700 dark:text-gray-300">{{ $row['present'] }}</td>
                                <td class="py-3.5 px-6 text-center text-gray-700 dark:text-gray-300">{{ $row['absent'] }}</td>
                                <td class="py-3.5 px-6 text-center text-gray-700 dark:text-gray-300">{{ $row['late'] }}</td>
                                <td class="py-3.5 px-6 text-center text-gray-700 dark:text-gray-300">{{ $row['excused'] }}</td>
                                <td class="py-3.5 px-6 text-center font-semibold text-gray-900 dark:text-gray-100">{{ $row['total'] }}</td>
                                <td class="py-3.5 px-6">
                                    @php
                                        $pct = $row['percentage'];
                                    ...
                                    <div class="flex items-center gap-2">
                                        <div class="flex-1 h-2 rounded-full bg-gray-100 dark:bg-gray-900 overflow-hidden">
                                            <div class="h-full rounded-full {{ $bar }}" style="width: {{ $pct }}%"></div>
                                        </div>
                                        <span class="text-xs font-semibold text-gray-700 dark:text-gray-300 w-9 text-right">{{ $pct }}%</span>
                                    </div>
                                </td>
                            </tr>
                        @empty
