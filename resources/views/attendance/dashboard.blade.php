<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">{{ __('Attendance Dashboard') }}</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Overview of attendance across all students') }} · {{ now()->format('l, M d, Y') }}</p>
            </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            @php
                $cards = [
                    ['label' => __('Overall Rate'), 'value' => $overallRate.'%', 'bg' => 'bg-emerald-50', 'text' => 'text-emerald-600', 'icon' => 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z'],
                    ['label' => __('Present Today'), 'value' => $today['present'], 'bg' => 'bg-green-50', 'text' => 'text-green-600', 'icon' => 'M5 13l4 4L19 7'],
                    ['label' => __('Absent Today'), 'value' => $today['absent'], 'bg' => 'bg-red-50', 'text' => 'text-red-600', 'icon' => 'M6 18L18 6M6 6l12 12'],
                    ['label' => __('Not Marked'), 'value' => $today['unmarked'], 'bg' => 'bg-gray-100', 'text' => 'text-gray-500', 'icon' => 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z'],
                ];
            @endphp
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
                <h3 class="text-base font-bold text-gray-900 dark:text-gray-100 mb-4">{{ __('Last 14 Days') }}</h3>
                <div class="h-64">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
                <h3 class="text-base font-bold text-gray-900 dark:text-gray-100 mb-4">{{ __('Status Distribution') }}</h3>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
                <h3 class="text-base font-bold text-gray-900 dark:text-gray-100 mb-4">{{ __('Today at a Glance') }}</h3>
                <div class="space-y-3">
                    @foreach(['present' => __('Present'), 'late' => __('Late'), 'excused' => __('Excused'), 'absent' => __('Absent')] as $key => $label)
                    ...
                    <div class="flex items-center justify-between border-t border-gray-100 dark:border-gray-700 pt-3">
                        <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('Marked / Total') }}</span>
            ...
            <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-base font-bold text-gray-900 dark:text-gray-100">{{ __('Recent Marks') }}</h3>
                    <a href="{{ route('attendance.history') }}" class="text-xs font-semibold text-blue-600 hover:underline">{{ __('View history') }}</a>
                </div>
                <div class="divide-y divide-gray-50 dark:divide-gray-700">
                    @forelse($recent as $record)
                        <div class="flex items-center justify-between py-2.5">
                            <a href="{{ route('attendance.student', $record->student_id) }}" class="text-sm font-medium text-gray-800 hover:text-blue-600 transition dark:text-gray-200 dark:hover:text-blue-400">
                                {{ $record->student->fullname ?? __('Unknown') }}
                            </a>
                    ...
                    @empty
                        <p class="text-sm text-gray-500 dark:text-gray-400 py-4 text-center">{{ __('No attendance recorded yet.') }}</p>
                    @endforelse
                </div>
            </div>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            new Chart(document.getElementById('trendChart'), {
                ...
                data: {
                    ...
                    datasets: [
                        { label: '{{ __('Present') }}', data: trendData.map(d => d.present), borderColor: '#16a34a', backgroundColor: 'rgba(22,163,74,0.1)', tension: 0.35, fill: true },
                        { label: '{{ __('Absent') }}', data: trendData.map(d => d.absent), borderColor: '#dc2626', backgroundColor: 'rgba(220,38,38,0.1)', tension: 0.35, fill: true },
                    ],
                },
