    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="{ }">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-gray-50/50">
                <h1 class="text-xl font-bold text-gray-900">Mark Attendance</h1>
                <p class="text-sm text-gray-500 mt-1">Select a date and set each student's status. Existing marks for the date are pre-filled.</p>
            </div>

            {{-- Date picker (GET reloads roster with existing marks) --}}
            <form method="GET" action="{{ route('attendance.mark') }}" class="px-6 py-4 border-b border-gray-100 flex items-end gap-3 m-0">
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Attendance Date</label>
                ...
                <button type="submit" class="bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-lg px-4 py-2 transition">Load</button>
            </form>

            @if($students->isEmpty())
                <div class="p-12 text-center">
                    <p class="text-sm text-gray-500">No students found. Add students before marking attendance.</p>
                </div>
            @else
                <form method="POST" action="{{ route('attendance.mark.store') }}" class="m-0">
                    ...
                    {{-- Quick set-all --}}
                    <div class="px-6 py-3 bg-blue-50/40 border-b border-gray-100 flex flex-wrap items-center gap-2 text-xs">
                        <span class="font-semibold text-gray-500 uppercase tracking-wider mr-1">Set all:</span>
                        ...
                        <button type="button"
                                ...
                                class="px-2.5 py-1 rounded-lg font-semibold bg-gray-100 text-gray-600 hover:opacity-80 transition">Clear</button>
                    </div>
                        ...
                        @foreach($students as $student)
                            <div class="flex items-center justify-between gap-4 px-6 py-3">
                                <div class="flex items-center gap-3 min-w-0">
                                    <div class="h-9 w-9 rounded-lg bg-gray-100 overflow-hidden shrink-0 border border-gray-100 flex items-center justify-center">
                                        @if($student->photo)
                                    ...
                                    <div class="min-w-0">
                                        <div class="text-sm font-medium text-gray-900 truncate">{{ $student->fullname }}</div>
                                        <div class="text-xs text-gray-400 truncate">{{ $student->course }}</div>
                                    </div>
                                </div>
                                <select name="statuses[{{ $student->id }}]" data-status
                                        class="border border-gray-200 text-gray-900 text-sm rounded-lg px-3 py-2 focus:ring-blue-500 focus:border-blue-500 shrink-0 w-36">
                    ...
                    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex items-center justify-between">
                        <p class="text-xs text-gray-500">{{ $students->count() }} students · Blank rows are skipped.</p>
