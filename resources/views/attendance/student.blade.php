<x-app-layout>
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            <div class="flex items-center gap-4">
                <div class="h-16 w-16 rounded-2xl bg-gray-100 overflow-hidden shrink-0 border border-gray-100 flex items-center justify-center">
                    @if($student->photo)
                ...
                <div class="min-w-0">
                    <h1 class="text-xl font-bold text-gray-900 truncate">{{ $student->fullname }}</h1>
                    <p class="text-sm text-gray-500">{{ $student->course }} · Attendance Record</p>
                </div>
                <div class="ml-auto text-right">
                    <div class="text-3xl font-bold {{ $percentage >= 75 ? 'text-green-600' : ($percentage >= 50 ? 'text-yellow-600' : 'text-red-600') }}">{{ $percentage }}%</div>
                    <div class="text-xs font-medium text-gray-400 uppercase tracking-wider">Attendance Rate</div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            @foreach(['present' => ['Present','text-green-600'], 'absent' => ['Absent','text-red-600'], 'late' => ['Late','text-yellow-600'], 'excused' => ['Excused','text-blue-600']] as $key => $meta)
                <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition">
                    ...
                    <div class="text-xs font-medium text-gray-400 uppercase tracking-wider mt-0.5">{{ $meta[0] }} Days</div>
                </div>
            @endforeach
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <h3 class="text-base font-bold text-gray-900 mb-4">This Month</h3>
            ...
            <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <h3 class="text-base font-bold text-gray-900 mb-4">Attendance Log ({{ $summary['total'] }} records)</h3>
                <div class="divide-y divide-gray-50 max-h-96 overflow-y-auto">
                    @forelse($records as $record)
                        <div class="flex items-center justify-between py-2.5">
                            <span class="text-sm text-gray-600">{{ $record->date->format('l, M d, Y') }}</span>
                            <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold {{ $record->status_badge }}">{{ $record->status_label }}</span>
                        </div>
                    @empty
                        <p class="text-sm text-gray-500 py-6 text-center">No attendance has been recorded yet.</p>
                    @endforelse
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (!total) { el.parentElement.innerHTML = '<p class="text-sm text-gray-400">No records this month.</p>'; return; }
