<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            <h1 class="text-xl font-bold text-gray-900 mb-4">Attendance History</h1>

            <form method="GET" action="{{ route('attendance.history') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 m-0">
                <div class="lg:col-span-2">
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Search Student</label>
                ...
                <div class="sm:col-span-2 lg:col-span-5 flex items-center gap-2">
                    ...
                    <a href="{{ route('attendance.history') }}" class="text-sm font-semibold text-gray-500 hover:text-gray-700 px-3 py-2">Reset</a>
                </div>
            </form>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-50 text-sm">
                        @forelse($records as $record)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="py-3.5 px-6 font-medium text-gray-700">{{ $record->date->format('M d, Y') }}</td>
                                <td class="py-3.5 px-6">
                                    <a href="{{ route('attendance.student', $record->student_id) }}" class="font-medium text-gray-900 hover:text-blue-600 hover:underline transition">
                                        {{ $record->student->fullname ?? 'Unknown' }}
                                    </a>
                                </td>
                                <td class="py-3.5 px-6 text-gray-500">{{ $record->student->course ?? '—' }}</td>
                                <td class="py-3.5 px-6 text-center">
                                    <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold {{ $record->status_badge }}">{{ $record->status_label }}</span>
                                </td>
                                <td class="py-3.5 px-6 text-right text-xs text-gray-400">{{ $record->created_at?->diffForHumans() }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="py-12 px-6 text-center text-sm text-gray-500">No attendance records match your filters.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
