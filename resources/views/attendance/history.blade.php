<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 mb-6">
            <h1 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">{{ __('Attendance History') }}</h1>

            <form method="GET" action="{{ route('attendance.history') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 m-0">
                <div class="lg:col-span-2">
                    <label class="block text-xs font-bold text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-1">{{ __('Search Student') }}</label>
                    <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="{{ __('Name...') }}"
                ...
                <div class="sm:col-span-2 lg:col-span-5 flex items-center gap-2">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg px-5 py-2 transition">{{ __('Filter') }}</button>
                    <a href="{{ route('attendance.history') }}" class="text-sm font-semibold text-gray-500 hover:text-gray-700 px-3 py-2 dark:text-gray-400 dark:hover:text-gray-200">{{ __('Reset') }}</a>
                </div>
            </form>
        </div>
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-gray-50/70 dark:bg-gray-900/70 border-b border-gray-100 dark:border-gray-700 text-gray-600 dark:text-gray-400 text-xs font-semibold uppercase tracking-wider">
                            ...
                            <th class="py-4 px-6">{{ __('Course') }}</th>
                            <th class="py-4 px-6 text-center">{{ __('Status') }}</th>
                            <th class="py-4 px-6 text-right">{{ __('Recorded') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50 dark:divide-gray-700 text-sm">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-12 px-6 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('No attendance records match your filters.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
