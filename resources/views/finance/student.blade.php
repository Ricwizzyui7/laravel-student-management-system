    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">Fee Records</h2>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                        <div>
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                Due: {{ $fee->due_date?->format('M d, Y') ?? 'No due date' }}
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            @endphp
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $badge }} capitalize">{{ $fee->status }}</span>
                            <a href="/finance/fees/{{ $fee->id }}/edit" class="inline-flex items-center justify-center text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950 h-8 w-8 rounded-lg transition" title="Edit">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            <form action="/finance/fees/{{ $fee->id }}" method="POST" class="inline m-0" onsubmit="return confirm('Delete this fee record and all associated payments?')">
                                @csrf
                                @method('DELETE')
                                <button class="inline-flex items-center justify-center text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950 h-8 w-8 rounded-lg transition" title="Delete">
                    ...
                    @if($fee->payments->count() > 0)
                        <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Payments ({{ $fee->payments->count() }})</span>
                                <a href="/finance/payments/{{ $fee->id }}/history" class="text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline">View All</a>
                            </div>
                            <div class="space-y-1">
        ...
        @empty
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-8 text-center">
                ...
                <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">No Fee Records</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 mb-4">This student has no fee assignments yet.</p>
                <a href="/finance/assign?student_id={{ $student->id }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-medium text-sm rounded-xl px-4 py-2.5 shadow-sm transition">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                    Assign First Fee
                </a>
