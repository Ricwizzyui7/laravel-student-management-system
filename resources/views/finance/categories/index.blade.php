    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">{{ __('Fee Categories') }}</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Manage fee types like tuition, hostel, food, etc.') }}</p>
                </div>
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-gray-50/70 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700 text-gray-600 dark:text-gray-400 text-xs font-semibold uppercase tracking-wider">
                            ...
                            <th class="py-4 px-6">{{ __('Description') }}</th>
                            <th class="py-4 px-6 text-center">{{ __('Assigned Fees') }}</th>
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                                ...
                                <td class="py-4 px-6 text-center">
                                    @if($category->is_active)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300">{{ __('Active') }}</span>
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">{{ __('Inactive') }}</span>
                                    @endif
                                </td>
                                <td class="py-4 px-6 text-right">
                                    <div class="inline-flex items-center gap-1">
                                        ...
                                        @if($category->student_fees_count === 0)
                                            <form action="/finance/categories/{{ $category->id }}" method="POST" class="inline m-0" onsubmit="return confirm('{{ __("Delete this category?") }}')">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tr>
                                <td colspan="5" class="py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('No Fee Categories') }}</h3>
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ __('Create your first fee category to get started.') }}</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
