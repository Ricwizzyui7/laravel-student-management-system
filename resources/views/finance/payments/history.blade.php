    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100">{{ __('Payment History') }}</h2>
                ...
                <div class="flex items-center gap-2">
                    <span class="text-xs font-semibold text-gray-400 uppercase">{{ __('Status') }}:</span>
                    @php
                    ...
                    @endphp
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $badge }} capitalize">{{ __(ucfirst($studentFee->status)) }}</span>
                </div>
            </div>
        </div>
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-gray-50/70 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700 text-gray-600 dark:text-gray-400 text-xs font-semibold uppercase tracking-wider">
                            ...
                            <th class="py-4 px-6 text-right">{{ __('Amount') }}</th>
                            <th class="py-4 px-6">{{ __('Method') }}</th>
                            <th class="py-4 px-6">{{ __('Reference Number') }}</th>
                            <th class="py-4 px-6">{{ __('Recorded By') }}</th>
                        ...
                        @forelse($studentFee->payments as $payment)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                                ...
                                <td class="py-4 px-6 text-right">
                                    <form action="/finance/payments/{{ $payment->id }}" method="POST" class="inline m-0" onsubmit="return confirm('{{ __("Delete this payment record?") }}')">
                                        @csrf
                                        @method('DELETE')
                            ...
                            <tr>
                                <td colspan="7" class="py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('No Payments Yet') }}</h3>
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ __('No payments have been recorded for this fee.') }}</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        @if($studentFee->payments->count() > 0)
            <div class="mt-6 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
                <div class="flex justify-between items-center">
                    <span class="text-sm font-semibold text-gray-500 dark:text-gray-400">{{ __('Total Paid') }}:</span>
                ...
                <div class="flex justify-between items-center mt-2">
                    <span class="text-sm font-semibold text-gray-500 dark:text-gray-400">{{ __('Remaining Balance') }}:</span>
