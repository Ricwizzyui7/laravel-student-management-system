<x-app-layout>
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
            <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">{{ __('Edit Fee Record') }}</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">{{ $studentFee->student?->fullname }} &mdash; {{ __('Current') }}: TSh {{ number_format($studentFee->amount, 2) }}</p>
            ...
            <form method="POST" action="/finance/fees/{{ $studentFee->id }}">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">{{ __('Status') }}</label>
                        <select name="status" required
                                class="w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 text-sm rounded-xl px-4 py-2.5 focus:ring-blue-500 focus:border-blue-500">
                            <option value="unpaid" @selected(old('status', $studentFee->status) === 'unpaid')>{{ __('Unpaid') }}</option>
                            <option value="partial" @selected(old('status', $studentFee->status) === 'partial')>{{ __('Partial') }}</option>
                            <option value="paid" @selected(old('status', $studentFee->status) === 'paid')>{{ __('Paid') }}</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-medium text-sm rounded-xl px-6 py-2.5 shadow-sm transition">{{ __('Update Fee') }}</button>
                    <a href="/finance/student/{{ $studentFee->student_id }}" class="text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition">{{ __('Cancel') }}</a>
                </div>
            </form>
        </div>
