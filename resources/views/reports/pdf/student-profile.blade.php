...
@section('content')
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 14px;">
        <tr>
            <td style="width: 90px; vertical-align: top;">
                ...
                @else
                    <div style="width: 80px; height: 96px; background: #e5e7eb; border-radius: 8px; text-align: center; line-height: 96px; color: #9ca3af; font-size: 9px;">{{ __('No Photo') }}</div>
                @endif
            </td>
    ...
    <h2 class="section">{{ __('Personal Information') }}</h2>
    <table class="data">
        <tbody>
            <tr><td style="width: 30%;"><strong>{{ __('Full Name') }}</strong></td><td>{{ $student->fullname }}</td></tr>
            <tr><td><strong>{{ __('Student Number') }}</strong></td><td>{{ $student->student_number }}</td></tr>
            <tr><td><strong>{{ __('Gender') }}</strong></td><td>{{ ucfirst($student->gender) }}</td></tr>
            <tr><td><strong>{{ __('Course') }}</strong></td><td>{{ $student->course ?? '—' }}</td></tr>
            <tr><td><strong>{{ __('Department') }}</strong></td><td>{{ $student->department_name ?? '—' }}</td></tr>
            <tr><td><strong>{{ __('Email') }}</strong></td><td>{{ $student->email ?? '—' }}</td></tr>
            <tr><td><strong>{{ __('Phone') }}</strong></td><td>{{ $student->phone ?? '—' }}</td></tr>
            <tr><td><strong>{{ __('Age') }}</strong></td><td>{{ $student->calculated_age ?? '—' }}</td></tr>
            <tr><td><strong>{{ __('Enrolled') }}</strong></td><td>{{ optional($student->created_at)->format('d M Y') ?? '—' }}</td></tr>
        </tbody>
    </table>

    <h2 class="section">{{ __('Attendance Summary') }}</h2>
    <table class="data">
        <thead>
            <tr>
                ...
                <th class="text-center">{{ __('Total Records') }}</th>
