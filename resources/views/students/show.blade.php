<x-app-layout>
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="px-6 sm:px-8 pb-6">
                <div class="flex flex-col sm:flex-row sm:items-end gap-4 -mt-14 sm:-mt-16">
                    ...
                    <div class="flex-1 min-w-0 sm:pb-1">
                        ...
                        <div class="flex flex-wrap items-center gap-2 mt-2">
                            ...
                            @if(strtolower($student->gender) === 'female')
                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-pink-50 text-pink-700 border border-pink-100/60">{{ __('Female') }}</span>
                            @else
                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-sky-50 text-sky-700 border border-sky-100/60">{{ __('Male') }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 sm:pb-1">
                        <a href="{{ route('finance.student', $student->id) }}" class="inline-flex items-center gap-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 text-sm font-semibold rounded-xl px-4 py-2.5 shadow-sm transition">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            @php
                $stats = [
                    ['label' => __('Attendance Rate'), 'value' => $attendanceRate.'%', 'icon' => 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z', 'bg' => 'bg-emerald-50', 'text' => 'text-emerald-600'],
                    ['label' => __('Present Days'), 'value' => $summary['present'], 'icon' => 'M5 13l4 4L19 7', 'bg' => 'bg-blue-50', 'text' => 'text-blue-600'],
                    ['label' => __('Profile Complete'), 'value' => $profileCompletion.'%', 'icon' => 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z', 'bg' => 'bg-indigo-50', 'text' => 'text-indigo-600'],
                    ['label' => __('Age'), 'value' => $student->calculated_age ?? 'N/A', 'icon' => 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z', 'bg' => 'bg-amber-50', 'text' => 'text-amber-600'],
                ];
            @endphp
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <h3 class="text-base font-bold text-gray-900">{{ __('Academic Information') }}</h3>
                    </div>
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('Course / Department') }}</dt>
                            <dd class="text-sm font-medium text-gray-900 mt-1">{{ $student->course ?? __('N/A') }}</dd>
                        </div>
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('Student ID') }}</dt>
                            <dd class="text-sm font-medium text-gray-900 mt-1">#{{ $student->id }}</dd>
                        </div>
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('Registration Date') }}</dt>
                            <dd class="text-sm font-medium text-gray-900 mt-1">{{ $student->created_at?->format('M d, Y') ?? 'N/A' }}</dd>
                        </div>
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('Last Updated') }}</dt>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <h3 class="text-base font-bold text-gray-900">{{ __('Personal Information') }}</h3>
                    </div>
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('Full Name') }}</dt>
                            <dd class="text-sm font-medium text-gray-900 mt-1">{{ $student->fullname }}</dd>
                        </div>
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('Gender') }}</dt>
                            <dd class="text-sm font-medium text-gray-900 mt-1">{{ $student->gender ?? __('N/A') }}</dd>
                        </div>
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('Date of Birth') }}</dt>
                            <dd class="text-sm font-medium text-gray-900 mt-1">{{ $student->date_of_birth?->format('M d, Y') ?? __('N/A') }}</dd>
                        </div>
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('Age') }}</dt>
                            <dd class="text-sm font-medium text-gray-900 mt-1">{{ $student->calculated_age ? $student->calculated_age.__(' years') : __('N/A') }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <h3 class="text-base font-bold text-gray-900">{{ __('Attendance Summary') }}</h3>
                    </div>

                    <div class="grid grid-cols-3 gap-4 text-center">
                        <div class="bg-green-50 p-4 rounded-xl">
                            <div class="text-2xl font-bold text-green-600">{{ $summary['present'] }}</div>
                            <div class="text-xs text-green-700 font-medium mt-0.5">{{ __('Present Days') }}</div>
                        </div>
                        <div class="bg-red-50 p-4 rounded-xl">
                            <div class="text-2xl font-bold text-red-600">{{ $summary['absent'] }}</div>
                            <div class="text-xs text-red-700 font-medium mt-0.5">{{ __('Absent Days') }}</div>
                        </div>
                        <div class="bg-yellow-50 p-4 rounded-xl">
                            <div class="text-2xl font-bold text-yellow-600">{{ $summary['late'] }}</div>
                            <div class="text-xs text-yellow-700 font-medium mt-0.5">{{ __('Late Days') }}</div>
                        </div>
                    </div>
                    ...
                    @if(auth()->user()->role === 'admin')
                        <form action="{{ route('attendance.store', $student->id) }}" method="POST"
                            ...
                            @csrf
                            <div class="flex-1">
                                <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">{{ __('Date') }}</label>
                            ...
                            <div class="flex-1">
                                <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">{{ __('Status') }}</label>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <h3 class="text-base font-bold text-gray-900 border-b border-gray-100 pb-3 mb-4">{{ __('Recent Attendance') }}</h3>
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    @if(auth()->user()->role === 'admin')
                                        <form action="{{ route('attendance.destroy', [$student->id, $record->id]) }}" method="POST" class="inline m-0"
                                              onsubmit="return confirm('{{ __('Remove this attendance record?') }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button class="text-gray-400 hover:text-red-600 text-xs font-semibold transition">{{ __('Remove') }}</button>
                                        </form>
                                    @endif
                                </div>
                            </div>
                        @empty
                            <p class="text-sm text-gray-500 py-4 text-center">{{ __('No attendance has been recorded for this student yet.') }}</p>
                        @endforelse
                    </div>
                </div>
            ...
            <div class="space-y-6">
                ...
                {{-- Profile Completion --}}
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <h3 class="text-base font-bold text-gray-900 mb-4">{{ __('Profile Completion') }}</h3>
                    <div class="flex items-center gap-4">
                        ...
                        <p class="text-xs text-gray-500 leading-relaxed">
                            @if($profileCompletion === 100)
                                {{ __('All key profile fields are complete.') }} 🎉
                            @else
                                {{ __('Add missing details like photo, email, phone, or date of birth to reach 100%.') }}
                            @endif
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <h3 class="text-base font-bold text-gray-900">{{ __('Contact Information') }}</h3>
                    </div>
                    <div class="space-y-3">
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-sky-50 transition group">
                            ...
                            <div class="min-w-0">
                                <div class="text-[11px] text-gray-400 uppercase tracking-wider font-semibold">{{ __('Email') }}</div>
                                <div class="text-sm font-medium text-gray-900 truncate">{{ $student->email ?? __('Not provided') }}</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-sky-50 transition group">
                            ...
                            <div class="min-w-0">
                                <div class="text-[11px] text-gray-400 uppercase tracking-wider font-semibold">{{ __('Phone') }}</div>
                                <div class="text-sm font-medium text-gray-900 truncate">{{ $student->phone ?? __('Not provided') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {{-- Recent Activities --}}
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <h3 class="text-base font-bold text-gray-900 mb-4">{{ __('Recent Activities') }}</h3>
                    <ol class="relative border-l border-gray-100 ml-2 space-y-5">
                        ...
                        @empty
                            <li class="ml-5"><p class="text-sm text-gray-500">{{ __('No recent activity.') }}</p></li>
                        @endforelse
                    </ol>
                </div>
