    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 sm:p-8 border-b border-gray-50 bg-gray-50/50">
                <h2 class="text-xl font-bold text-gray-900">Modify Student Profile</h2>
                <p class="text-sm text-gray-500 mt-1">Updating administrative fields for student reference path ID: #{{ $student->id }}.</p>
            </div>
            ...
            <form action="/students/{{ $student->id }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6 m-0">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                    ...
                    <div class="flex flex-col items-center text-center p-4 bg-gray-50/60 rounded-xl border border-dashed border-gray-200">
                        <span class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Profile Photo</span>
                        <div class="relative h-28 w-28 rounded-full bg-white shadow-inner border border-gray-100 overflow-hidden flex items-center justify-center">
                            @if($student->photo)
                                <img id="profileImagePreview" src="{{ asset('storage/'.$student->photo) }}" class="h-full w-full object-cover">
                                <svg id="defaultAvatarIcon" class="hidden w-14 h-14 text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <p class="text-[11px] text-gray-400 mt-2">Currently assigned image framework preview tier.</p>
                    </div>
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Change Student Photo (Optional)</label>
                    <input type="file"
                           name="photo"
