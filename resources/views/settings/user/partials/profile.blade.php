        ...
        <form method="POST" action="{{ route('settings.user.profile-picture') }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="flex items-center gap-6">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Max 5 MB. JPG, PNG, GIF</p>
    ...
    <div class="border-t pt-6">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" value="{{ $user->name }}" disabled class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-500">
            </div>
            <form method="POST" action="{{ route('settings.user.email') }}" class="space-y-4">
                @csrf
                <div>
                    <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Email Address</label>
                    <input id="email" name="email" type="email" value="{{ $user->email }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
