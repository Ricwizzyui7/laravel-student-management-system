...
<div x-data="{ open: false }" class="relative">
    {{-- Bell trigger --}}
    <button @click="open = !open" class="relative inline-flex items-center justify-center h-10 w-10 rounded-full text-gray-500 hover:text-gray-700 hover:bg-gray-100 focus:outline-none transition">
    ...
    <div x-show="open"
         ...
         class="absolute right-0 mt-2 w-80 sm:w-96 max-w-[calc(100vw-2rem)] origin-top-right rounded-2xl bg-white shadow-xl border border-gray-100 z-50 overflow-hidden"
         style="display: none;">

        {{-- Header --}}
        <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100">
            <h3 class="text-sm font-bold text-gray-900">Notifications
                @if($unreadCount > 0)<span class="ml-1 text-xs font-medium text-gray-400">({{ $unreadCount }} unread)</span>@endif
            </h3>
            @if($unreadCount > 0)
        ...
        <div class="max-h-96 overflow-y-auto divide-y divide-gray-50">
            @forelse($notifications as $note)
                ...
                @endphp
                <div class="flex items-start gap-3 px-4 py-3 {{ $isUnread ? 'bg-blue-50/40' : '' }} hover:bg-gray-50 transition group">
                    ...
                    <div class="min-w-0 flex-1">
                        <form method="POST" action="{{ route('notifications.read', $note->id) }}" class="m-0">
                            ...
                            <button type="submit" class="text-left w-full">
                                ...
                                <div class="text-xs text-gray-500 mt-0.5 leading-snug">{{ $note->data['message'] ?? '' }}</div>
                                <div class="text-[11px] text-gray-400 mt-1">{{ $note->created_at->diffForHumans() }}</div>
                            </button>
                        </form>
                    </div>
            ...
            @empty
                <div class="px-4 py-10 text-center">
                    <svg class="h-10 w-10 mx-auto text-gray-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                    <p class="text-sm text-gray-400 mt-2">No notifications yet</p>
                </div>
            @endforelse
        </div>
