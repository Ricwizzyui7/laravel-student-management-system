    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">{{ __('Course Catalog') }}</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Manage academic programs, departments, and enrolment.') }}</p>
                </div>
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                    <form method="GET" action="{{ route('courses.index') }}" class="relative w-full sm:w-72 m-0">
                        <input type="text" name="search" value="{{ $search ?? '' }}"
                               ...
                               placeholder="{{ __('Search code, name, department...') }}">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            @php
                $cards = [
                    ['label' => __('Total Courses'), 'value' => $stats['total'], 'bg' => 'bg-blue-50', 'text' => 'text-blue-600', 'icon' => 'M12 14l9-5-9-5-9 5 9 5z M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z'],
                    ['label' => __('Departments'), 'value' => $stats['departments'], 'bg' => 'bg-indigo-50', 'text' => 'text-indigo-600', 'icon' => 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4'],
                    ['label' => __('Active (with students)'), 'value' => $stats['withStudents'], 'bg' => 'bg-emerald-50', 'text' => 'text-emerald-600', 'icon' => 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z'],
                ];
            @endphp
        ...
        @if($courses->isEmpty())
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-12 text-center">
                ...
                <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('No courses found') }}</h3>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $search ? __('Try a different search term.') : __('Add your first course to get started.') }}</p>
            </div>
        @else
                        ...
                        <div class="p-5 flex-1">
                            ...
                            <div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400">
                                @if($course->department_name)
                                    <span class="inline-flex items-center gap-1"><svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5"/></svg>{{ $course->department_name }}</span>
                                @endif
                                @if($course->duration)
...
<div class="px-5 py-3 border-t border-gray-50 dark:border-gray-700 bg-gray-50/40 dark:bg-gray-900/50 flex items-center justify-between">
                            <a href="{{ route('courses.show', $course->id) }}" class="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline">{{ __('View Details') }}</a>
                            @if(Auth::user()?->role == 'admin')
                                <div class="flex items-center gap-1">
                                    <a href="{{ route('courses.edit', $course->id) }}" class="text-xs font-semibold text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950 px-2.5 py-1 rounded-lg transition">{{ __('Edit') }}</a>
                                    <form action="{{ route('courses.destroy', $course->id) }}" method="POST" class="inline m-0" onsubmit="return confirm('{{ __('Delete this course?') }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button class="text-xs font-semibold text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950 px-2.5 py-1 rounded-lg transition">{{ __('Delete') }}</button>
                                    </form>
                                </div>
                            @endif
                        </div>
