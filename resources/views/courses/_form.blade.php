...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
    <div>
        <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">{{ __('Course Code') }}</label>
    ...
    <div class="sm:col-span-2">
        <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">{{ __('Course Name') }}</label>
        <input type="text" name="name" value="{{ old('name', $course->name ?? '') }}"
               ...
               placeholder="{{ __('e.g. Cyber Security') }}">
...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
    ...
    <div>
        ...
        <input type="text" name="duration" value="{{ old('duration', $course->duration ?? '') }}"
               class="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500 text-gray-900 dark:text-gray-100 text-sm rounded-xl px-4 py-2.5 transition"
               placeholder="{{ __('e.g. 3 Years') }}">
        @error('duration') <p class="text-xs text-red-600 mt-1 font-medium">{{ $message }}</p> @enderror
    </div>
</div>

<div>
    <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">{{ __('Description') }} <span class="text-gray-400 dark:text-gray-500 font-medium normal-case">({{ __('optional') }})</span></label>
    <textarea name="description" rows="4"
              class="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500 text-gray-900 dark:text-gray-100 text-sm rounded-xl px-4 py-2.5 transition"
              placeholder="{{ __('Brief overview of the course...') }}">{{ old('description', $course->description ?? '') }}</textarea>
